...
struct ContentView: View {
    ...
    @State private var logiran = false
    ...
    func authenticateUser() {
        ...
        if context.canEvaluatePolicy(.deviceOwnerAuthenticationWithBiometrics, error: &error) {
            let reason = "logiraj se so FaceID ili TouchID"
            ...
            context.evaluatePolicy(.deviceOwnerAuthenticationWithBiometrics, localizedReason: reason) { success, error in
                DispatchQueue.main.async {
                    if success {
                        ...
                        print("login so biometrika failed: \(error?.localizedDescription ?? "Unknown error")")
                    }
                }
            }
        } else if context.canEvaluatePolicy(.deviceOwnerAuthentication, error: &error) {
            let reason = "logiraj se so pin"
            ...
            context.evaluatePolicy(.deviceOwnerAuthentication, localizedReason: reason) { success, error in
                DispatchQueue.main.async {
                    if success {
                        ...
                        print("login so pin failed: \(error?.localizedDescription ?? "Unknown error")")
                    }
                }
            }
        } else {
            print("ne mozhe da se logira: \(error?.localizedDescription ?? "Unknown error")")
        }
    }
}
